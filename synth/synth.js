...
let waveform =
  waveformControlSine.value ||
  waveformControlSquare.value ||
  waveformControlTriangle.value ||
  waveformControlSawtooth.value;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('keyup', (e) => {
    // start recording from keyboard
    if (!keyOff) {
      // allow r key for record if user isn't typing in the song name field
      const x = e.keyCode;
  ...
  function SaveSong(name, layerToStore) {
    // object constructor for new song
    this.name = name;
  ...
  function playStoredMusic(musicalLayer) {
    ...
    let convolverTimePB = 1.0;
    let convolverEffectPB = (function () {
      let convolverPB = contextPlayback.createConvolver(),
        noiseBufferPB = contextPlayback.createBuffer(
          2,
          convolverTimePB * contextPlayback.sampleRate,
          contextPlayback.sampleRate
        ),
    ...
    for (let i = 0; i < musicalLayer.length; i++) {
      // loop through the song's scheduled events
      const currentNoteValue = musicalLayer[i];
      if (currentNoteValue.note_switch === 144) {
        // if saved musical note is being pressed down

        const oscillatorPlayback = contextPlayback.createOscillator();

        activeOscillatorsPlayback[
          currentNoteValue.note_name
        ] = oscillatorPlayback;
        ...
        if (currentNoteValue.note_verb) {
          // check if verb is on during recording
        ...
        oscillatorPlayback.frequency.setValueAtTime(
          currentNoteValue.note_name,
          currentNoteValue.note_time * (1 / playbackMultiplier)
        ); // schedule frequency of current note
        envelopePlayback.gain.setValueAtTime(
          currentNoteValue.note_gain,
          currentNoteValue.note_time * (1 / playbackMultiplier)
        ); // schedule gain level of current note
      } else if (currentNoteValue.note_switch === 128) {
        // if saved musical note is being lifted up
        const oscillatorPlayback =
          activeOscillatorsPlayback[currentNoteValue.note_name];
        oscillatorPlayback.frequency.setValueAtTime(
          0,
          currentNoteValue.note_time * (1 / playbackMultiplier)
        );
        envelopePlayback.gain.setValueAtTime(
          0,
          currentNoteValue.note_time * (1 / playbackMultiplier)
        ); // schedule note to turn off
      }
    }
    if (loopToggle.checked && recordPlayButton.checked) {
      // if loop toggle is checked, restart play function with same song after last scheduled event
      setTimeout(
        () => playStoredMusic(musicalLayer),
        loopTime * (1 / playbackMultiplier) * 1000
      );
    }
  }
});
...
function updateSongs() {
  ...
  for (let i = 0; i < usersSongs.length; i++) {
    // loop through the user's songs and create selectable list on dom
    const newOption = document.createElement('option');
